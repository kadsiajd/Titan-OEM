        ...
        {categories && (
          <div className="relative mt-12">
            <button
              ...
              className="absolute left-0 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white p-2 text-gray-700 shadow-md disabled:opacity-40 sm:hidden"
            >
            ...
            <div
              ...
              className="flex snap-x snap-mandatory gap-6 overflow-x-auto pb-4 text-left [-ms-overflow-style:none] [scrollbar-width:none] sm:grid sm:grid-cols-2 sm:gap-8 sm:overflow-visible sm:pb-0 lg:grid-cols-3 [&::-webkit-scrollbar]:hidden"
            >
              {categories.map((category) => (
                <div key={category.id} className="w-72 shrink-0 snap-start sm:w-auto">
              ...
              {categories.map((category, index) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${
                    index === activeIndex ? 'w-6 bg-brand-600' : 'w-2 bg-gray-300'
                  }`}
                />
              ))}
            </div>
